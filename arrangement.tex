...
\[ 
\text{Maximize}\quad Z = \left( \sum_{i \in I} \sum_{j \in J} a_i F(t_{ij}) y_{ij}, -\sum_{j \in J_1}q_{j} \right) 
\]
\[
    \text{subject to: }
    \begin{cases}
		y_{ij} \leq c_j +  h_j \leq 1  & \forall i \in I \quad \forall j \in J \\
		\sum_{k \in M_j}h_k - c_j + q_{j} \geq 0 & \forall j \in J_1 \\
		\sum_{j \in J_1} c_j = P_c \quad \sum_k h_k = P_h \\
		\sum_{j \in J} y_{ij} = 1 \quad \sum_{k \in J_2} q_{jk} = 1 & \forall i \in I \quad \forall j \in J \\
		c_j,h_k,y_{ij},q_{jk} \in \{0,1\}
	\end{cases}
\]
\end{definition}
